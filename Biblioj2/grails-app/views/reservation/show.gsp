	...
	<body>
		...
		<div id="show-reservation" class="content scaffold-show" role="main">
			...
			<ol class="property-list reservation">
				...
				<g:if test="${reservationInstance?.livresReserves}">
				<li class="fieldcontain">
					<span id="livresReserves-label" class="property-label"><g:message code="reservation.livresReserves.label" default="Livres Reserves" /></span>
					
						<g:each in="${reservationInstance.livresReserves}" var="l">
						<span class="property-value" aria-labelledby="livresReserves-label"><g:link controller="livreReserve" action="show" id="${l.id}">${l?.encodeAsHTML()}</g:link></span>
						</g:each>
